...
export async function buyerRegistration(
  ...
  while (true) {
    ...
    if (!contactCtx.message?.contact) {
      await ctx.reply(
        "❌ Необходимо отправить контакт для завершения регистрации. Попробуйте ещё раз."
      );
      continue;
    }
...
export async function sellerRegistration(
  ...
  await ctx.reply(
    "<b>🏪 Регистрация продавца</b>\n\n<b>Шаг 1/4:</b> Пожалуйста, поделитесь своим номером телефона:",
    {
      parse_mode: "HTML",
      reply_markup: new Keyboard()
        .requestContact("📱 Поделиться номером телефона")
  ...
  while (true) {
    await ctx.reply(
      `<b>✅ Магазин: ${shopName}</b>\n\n<b>Шаг 3/4:</b> Введите номер карты для получения выплат (16 цифр):`,
      { parse_mode: "HTML" }
    );
  ...
  while (true) {
    await ctx.reply(
      `<b>✅ Карта: ${cardNumber.slice(0, 4)} **** **** ${cardNumber.slice(
        -4
      )}</b>\n\n<b>Шаг 4/4:</b> Введите описание вашего магазина (что продаете, особенности):`,
      { parse_mode: "HTML" }
    );
    const descriptionCtx = await conversation.wait();
    if (!descriptionCtx.message?.text) {
      await ctx.reply(
        "❌ Необходимо ввести описание магазина. Попробуйте ещё раз."
      );
      continue;
    }
  ...
  if (process.env.ADMIN_ID) {
    try {
      ...
      console.log(
        `📤 Отправка уведомления админу (ID: ${process.env.ADMIN_ID}) о магазине ${shop._id}`
      );
      ...
      await ctx.api.sendMessage(
        ...
        `🆕 Новый продавец!\n\n` +
          ...
          `🏪 ${shopName}\n` +
          `📝 ${description}\n` +
          `💳 Карта: ${cardNumber.slice(0, 4)} **** **** ${cardNumber.slice(
            -4
          )}\n` +
...
export async function searchProducts(
  ...
  const searchResults = allResults.filter((p) => {
    ...
    return (
      seller && seller._id && seller._id.toString() !== user._id.toString()
    );
  });
  ...
  const showProduct = async (
    index: number,
    deleteOldMessages: boolean = false,
    ...
    if (product.location?.latitude && product.location?.longitude) {
      keyboard
        .text("📍 Показать на карте", `search_location:${product._id}`)
        .row();
    }
    ...
    if (mediaItems.length > 1) {
      try {
        ...
        const mediaGroup: Array<
          ReturnType<
            typeof InputMediaBuilder.photo | typeof InputMediaBuilder.video
          >
        > = [];
        ...
        for (let i = 0; i < mediaItems.length && i < 10; i++) {
          ...
          if (media.mediaType === "video") {
            mediaGroup.push(
              InputMediaBuilder.video(media.fileId, {
            ...
            mediaGroup.push(
              InputMediaBuilder.photo(media.fileId, {
                caption: i === 0 ? message : undefined,
                parse_mode: i === 0 ? "HTML" : undefined,
              })
            );
  ...
  while (true) {
    ...
    if (data?.startsWith("search_prev:")) {
      if (currentIndex > 0) {
        currentIndex--;
        currentMessageIds = await showProduct(
          currentIndex,
          true,
          currentMessageIds
        );
    ...
    if (data?.startsWith("search_add_cart:")) {
      ...
      await callbackCtx.answerCallbackQuery();
      try {
        const cartService = (await import("../database/controllers/cart"))
          .default;
        await cartService.addToCart(ctx.from!.id, productId, 1);
        await ctx
          .reply("🛒 Товар добавлен в корзину", {
            reply_markup: { remove_keyboard: true },
          })
          .then((msg) => {
            setTimeout(() => {
    ...
    if (data?.startsWith("search_location:")) {
      ...
      try {
        ...
        await callbackCtx.answerCallbackQuery(
          "❌ Ошибка при отправке геолокации"
        );
      }
      continue;
    }
