  ...
  while (true) {
    ...
    if (data === "catalog_exit") {
      ...
      await deleteMessages(
        ctx,
        [...currentMessageIds, categoriesMessageId].filter(Boolean) as number[]
      );
    ...
    if (data.startsWith("catalog_category:")) {
      ...
      await callbackCtx.answerCallbackQuery(
        `📂 Загрузка товаров из категории ${currentCategory.name}...`
      );
    ...
    if (data === "catalog_next") {
      if (currentIndex < categoryProducts.length - 1) {
        ...
        await callbackCtx.answerCallbackQuery(
          "📍 Это последний товар в категории"
        );
      }
      continue;
    }
    ...
    if (data.startsWith("catalog_buy_now:")) {
      ...
      try {
        ...
        if (product.price < MIN_PAYMENT_AMOUNT) {
          await ctx.reply(
            `❌ Минимальная сумма для оплаты: ${MIN_PAYMENT_AMOUNT} ₽\nЦена товара: ${product.price} ₽`
          );
          continue;
        }
        ...
        const commission = Math.round(
          (product.price * COMMISSION_PERCENT) / 100
        );
        ...
        await ctx.reply(
          "✅ Счет отправлен. Нажмите 'Оплатить' для завершения покупки."
        );
      } catch (err: unknown) {
...
function buildProductKeyboard(
  ...
  if (hasLocation) {
    keyboard
      .row()
      .text("📍 Показать на карте", `catalog_location:${product._id}`);
  }
...
async function sendProductMedia(
  ...
  if (mediaItems.length > 1) {
      ...
      if (firstMedia?.fileId) {
        const method =
          firstMedia.mediaType === "video"
            ? "replyWithVideo"
            : "replyWithPhoto";
        const sentMsg = await ctx[method](firstMedia.fileId, {
          caption: message,
          parse_mode: "HTML",
          reply_markup: keyboard,
        });
    ...
    const method =
      mediaItems[0].mediaType === "video" ? "replyWithVideo" : "replyWithPhoto";
    const sentMsg = await ctx[method](mediaItems[0].fileId, {
      caption: message,
      parse_mode: "HTML",
      reply_markup: keyboard,
    });
