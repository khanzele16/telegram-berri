...
// Обработка кнопки "Перейти в продавцы"
...
// Обработка кнопки "Стать продавцом"
export const handleBecomeSeller = async (ctx: MyContext) => {
  ...
  // Запускаем разговор регистрации продавца
...
export const handleProfile = async (ctx: MyContext) => {
  ...
  profileText += `Активный профиль: ${currentProfile === 'buyer' ? '👤 Покупатель' : '🏪 Продавец'}\n\n`;
  ...
  if (user.profiles.seller.isActive && user.profiles.seller.shopId) {
    const shop = user.profiles.seller.shopId as unknown as { name: string; description: string; productsCount: number; salesCount: number; rating: number; reviewsCount: number; isApproved: boolean };
    ...
    profileText += `⭐ Рейтинг: ${shop.rating.toFixed(1)} (${shop.reviewsCount} отзывов)\n`;
    profileText += `✅ Статус: ${shop.isApproved ? 'Одобрен' : 'На модерации'}\n`;
  }
  
  await ctx.reply(profileText);
};

// Обработка кнопки "Лента"
export const handleFeed = async (ctx: MyContext) => {
  ...
  // Запускаем conversation для ленты товаров
...
// Обработка кнопки "Мои товары"
export const handleMyProducts = async (ctx: MyContext) => {
  if (!ctx.from) return;
  
  // Запускаем conversation для просмотра товаров
...
export const handleAddProduct = async (ctx: MyContext) => {
  ...
  // Запускаем conversation для добавления товара
  // Проверки будут внутри conversation
  await ctx.conversation.enter('addProduct');
};

// Обработка кнопки "Заказы" (для продавца)
export const handleSellerOrders = async (ctx: MyContext) => {
  ...
  // Проверяем, одобрен ли магазин
  const shop = user.profiles.seller.shopId as unknown as { isApproved: boolean } | null;
...
export const handleStatistics = async (ctx: MyContext) => {
  ...
  const shop = user.profiles.seller.shopId as unknown as { isApproved: boolean; productsCount: number; salesCount: number; totalRevenue: number; rating: number; reviewsCount: number } | null;
  ...
  if (!isApproved) {
    ...
    stats += `⭐ <b>Рейтинг:</b> ${shop.rating?.toFixed(1) || '0.0'} (${shop.reviewsCount || 0} отзывов)`;
  }
...
export const handleSettings = async (ctx: MyContext) => {
  ...
  const shop = user.profiles.seller.shopId as unknown as { name: string; description: string };
